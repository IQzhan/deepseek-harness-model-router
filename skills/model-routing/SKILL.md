...
- 想给你自己的任务精确划线，就在设置页用**工具范围**（它读的是 Host 的真实工具表），别凭记忆写名字。

**两种写法不是同一个意思，别混。** 应用时两者都归结为一张白名单（只有白名单能摘掉子会话自己预设里的
工具），但含义不同：

| 写法 | 含义 | 以后 DSH 新增的工具 |
| --- | --- | --- |
| `{ allow: [read, write] }` | 固定白名单：只给这些 | **不会**自动获得，要回来自己加 |
| `{ deny: [pwsh] }` | 继承来的工具里去掉这些 | **会**自动获得 |

设置页的「工具范围」下拉就是这两者加「不限制」，并且会**照原样写回**——一个 `deny` 文档保存后还是
`deny`。同时写 `allow` 和 `deny` 时不报错，但 `deny` 从 `allow` 里减掉（会有一条校验警告）；空列表
在这里等于"不限制"，不等于"什么都不给"。
...
- **不要留下非法状态**：`defaultTaskId` 必须指向一个启用且池非空的任务，否则等于没有默认任务；
  任务 id 不得重复；`childTools` 不能是空对象（`{}` 会被拒绝），也不能是空列表（那等于不限制，不是"不给工具"）。
